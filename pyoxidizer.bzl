...
def resource_callback(policy, resource):
    ...
    if res_type == "PythonModuleSource" or res_type == "PythonPackageResource":
        # 基于 `policy.include_distribution_sources = False` 和 `policy.include_non_distribution_sources = False` 的白名单
        res_name = resource.name if res_type == "PythonModuleSource" else resource.package if res_type == "PythonPackageResource" else ""
        is_test = check_module((
            "ctypes.test",
            "distutils.tests",
            "idlelib.idle_test",
            "lib2to3.tests",
            ...
            "tkinter.test",
            "unittest.test",
        ), res_name)  # make `policy.include_test = False` great again
        ...
        if not is_test and not is_useless:
            resource.add_include = True
    elif res_type == "PythonExtensionModule":
        # 暂时无效 https://github.com/indygreg/PyOxidizer/pull/456
    ...
    elif res_type == "PythonPackageDistributionResource":
        # 排除所有 .dist-info 目录
        resource.add_include = False
    else:
        print("warning: resource_callback 存在未知类型", res_type)
        pass
...
def make_exe():
    ...
    python_config.run_command = "from eaio.__main__ import main; main()"

    # Run a Python module as __main__ when the interpreter starts.
    # python_config.run_module = "eaio"
